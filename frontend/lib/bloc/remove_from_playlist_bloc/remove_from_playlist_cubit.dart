...
class RemoveFromPlaylistBloc extends Bloc<RemoveFromPlaylistEvent, RemoveFromPlaylistState> {
  ...
  RemoveFromPlaylistBloc(this._tracksRepository, this._playlistsRepository) : super(RemoveFromPlaylistInitial()) {
    on<TrackChoosen>(_onTrackChoosen);
    on<ConfirmationReceived>(_onConfirmationReceived);
  }

  FutureOr<void> _onTrackChoosen(TrackChoosen event, Emitter<RemoveFromPlaylistState> emit) {
    if (event.trackId.isEmpty || event.playlistId == null || event.playlistId!.isEmpty) {
  ...
  FutureOr<void> _onConfirmationReceived(event, Emitter<RemoveFromPlaylistState> emit) async {
    try {
      final result = await _playlistsRepository.removeTrackFromPlaylist(event.playlistId, event.trackId);
      ...
      if (result) {
        _playlistsRepository.items
            ...
            .tracks
            .remove(_tracksRepository.items.firstWhere((element) => element.id == event.trackId));
