...
class AlbumCoverCubit extends Cubit<AlbumCoverState> {
  ...
  AlbumCoverCubit({required this.albumsRepository, required this.albumId}) : super(const AlbumCoverState.initial());

  Future<void> search({String query = ''}) async {
    emit(const AlbumCoverState.searching());
    try {
      final candidates = await albumsRepository.searchCover(albumId, query: query);
